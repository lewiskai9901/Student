 ...
 * 1. 管理员登录成功 > 跳转到 /inspection (默认 hub) 且侧边栏加载完成
 * 2. 错误凭据 > 显示错误提示且仍停留在 /login
 * 3. 登录 > 点击用户菜单 > 退出登录 > 返回 /login
 *
 * 2026-05-01 起 (commit 6c85a261) 根路由从 /my/dashboard 改成 /inspection,
 * 检查平台 Hub 成为非教师用户登录后第一屏.
 *
...
test.describe('Critical auth flows', () => {
  test('admin login + reach inspection hub', async ({ page }) => {
    ...
    // 登录成功后非教师走 router.push('/') → 根路由重定向到 /inspection
