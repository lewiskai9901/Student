...
const BASE = '/inspection/analytics'

// ==================== Daily ====================

export function getDailyRanking(projectId: LongId, date: string): Promise<DailySummary[]> {
  return http.get<DailySummary[]>(`${BASE}/daily-ranking`, { params: { projectId, date } })
}

export function getDailySummary(projectId: LongId, date: string): Promise<DailySummary[]> {
  return http.get<DailySummary[]>(`${BASE}/daily-summary`, { params: { projectId, date } })
}

// ==================== Period ====================

export function getPeriodSummary(projectId: LongId, periodType: PeriodType, periodStart: string): Promise<PeriodSummary[]> {
  return http.get<PeriodSummary[]>(`${BASE}/period-summary`, { params: { projectId, periodType, periodStart } })
}

// ==================== Trend ====================

export function getTrend(projectId: LongId, targetType: string, targetId: LongId, startDate: string, endDate: string): Promise<DailySummary[]> {
  return http.get<DailySummary[]>(`${BASE}/trend`, { params: { projectId, targetType, targetId, startDate, endDate } })
}

// ==================== Comparison ====================

export function getComparison(projectId: LongId, date: string): Promise<DailySummary[]> {
  return http.get<DailySummary[]>(`${BASE}/comparison`, { params: { projectId, date } })
}

// ==================== Dimension Breakdown ====================

export function getDimensionBreakdown(projectId: LongId, startDate: string, endDate: string): Promise<DailySummary[]> {
  return http.get<DailySummary[]>(`${BASE}/dimension-breakdown`, { params: { projectId, startDate, endDate } })
}

// ==================== Inspector Performance ====================

export function getInspectorPerformance(projectId: LongId, periodType: PeriodType, periodStart: string): Promise<PeriodSummary[]> {
  return http.get<PeriodSummary[]>(`${BASE}/inspector-performance`, { params: { projectId, periodType, periodStart } })
}

// ==================== Corrective Summary ====================

export function getCorrectiveSummary(projectId: LongId, periodType?: string, periodStart?: string): Promise<CorrectiveSummary> {
...
export function getInspectorSummaries(projectId: LongId, periodType: string, periodStart: string): Promise<InspectorSummary[]> {
  return http.get<InspectorSummary[]>(`${BASE}/inspector-summaries`, { params: { projectId, periodType, periodStart } })
}

// ==================== Item Frequency / Pareto ====================

export function getItemFrequencies(projectId: LongId, periodType: string, periodStart: string): Promise<ItemFrequencySummary[]> {
  return http.get<ItemFrequencySummary[]>(`${BASE}/item-frequency`, { params: { projectId, periodType, periodStart } })
}

export function getPareto(projectId: LongId, periodType: string, periodStart: string, limit?: number): Promise<ItemFrequencySummary[]> {
  return http.get<ItemFrequencySummary[]>(`${BASE}/pareto`, { params: { projectId, periodType, periodStart, limit } })
}

// ==================== Corrective Period Summary ====================

export function getCorrectiveSummaryByPeriod(projectId: LongId, periodType: string, periodStart: string): Promise<CorrectivePeriodSummary> {
  return http.get<CorrectivePeriodSummary>(`${BASE}/corrective-summary`, { params: { projectId, periodType, periodStart } })
}

export function getCorrectiveSummaries(projectId: LongId): Promise<CorrectivePeriodSummary[]> {
