...
const BASE = '/inspection/compliance-standards'
...
export function getMappings(itemId: number): Promise<ItemComplianceMapping[]> {
  return http.get<ItemComplianceMapping[]>(`/inspection/template-items/${itemId}/compliance-mappings`)
}

export function createMapping(itemId: number, data: CreateMappingRequest): Promise<ItemComplianceMapping> {
  return http.post<ItemComplianceMapping>(`/inspection/template-items/${itemId}/compliance-mappings`, data)
}

export function deleteMapping(itemId: number, mappingId: number): Promise<void> {
  return http.delete(`/inspection/template-items/${itemId}/compliance-mappings/${mappingId}`)
}
