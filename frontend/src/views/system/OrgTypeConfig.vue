  ...
  <div class="flex h-full bg-gray-50">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <div class="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
        <div>
          <h1 class="text-lg font-semibold text-gray-900">组织类型配置</h1>
          <p class="mt-0.5 text-sm text-gray-500">配置组织层级类型，定义分类、行为特征与层级结构</p>
        </div>
        <button
          @click="loadData"
      ...
      <div class="flex-1 overflow-y-auto px-6 pt-5 pb-6">
        ...
        <div v-if="selectedType" class="space-y-5">
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    >
                      {{ ORG_CATEGORY_LABELS[selectedType.category] || selectedType.category }}
                    </span>
          ...
          <!-- Cross References -->
          <div
            v-if="(selectedType.defaultUserTypeCodes?.length || 0) + (selectedType.defaultPlaceTypeCodes?.length || 0) > 0"
          ...
          <!-- Position Templates -->
          <div
            v-if="selectedType.defaultPositions?.length"
            class="rounded-xl border border-gray-200 bg-white p-4"
          >
            <h3 class="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-400">岗位模板</h3>
            <p class="mb-2 text-xs text-gray-400">创建该类型的组织时，用户可从这些岗位中选择要启用的</p>
            <div class="flex flex-wrap gap-1.5">
              <span
                v-for="(pos, idx) in selectedType.defaultPositions"
                :key="idx"
                class="inline-flex items-center gap-1 rounded-md bg-gray-50 px-2.5 py-1.5 text-sm text-gray-700"
              >
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 text-gray-400">
          <Layers class="h-12 w-12 text-gray-300" />
      ...
      <div class="relative w-[480px] bg-white shadow-xl flex flex-col animate-slide-in-right">
        ...
        <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div>
            ...
            <p v-if="!isEdit && addingUnderParentCode" class="mt-0.5 text-xs text-gray-400">
              创建后将自动加入「{{ getParentName(addingUnderParentCode) }}」的允许子类型
            </p>
          </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <!-- Code (auto-generated, shown as readonly tag) -->
          <div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div
                v-for="(label, key) in ORG_FEATURE_LABELS"
                :key="key"
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-xs font-medium text-gray-500">岗位模板</label>
              <button
                type="button"
                @click="addPositionTemplate"
                class="flex items-center gap-1 text-[11px] text-blue-500 hover:text-blue-600 transition-colors"
              >
                <Plus class="h-3 w-3" />
                添加岗位
              </button>
            ...
            <p class="mb-2 text-[11px] text-gray-400">
              定义该类型组织可以拥有的岗位，创建组织时用户可从中选择
            </p>
            <div v-if="formData.defaultPositions.length === 0" class="rounded-lg bg-gray-50 px-3 py-4 text-center text-xs text-gray-400">
              暂无岗位模板
            </div>
            <div v-else class="space-y-2">
              <div
                v-for="(pos, idx) in formData.defaultPositions"
                :key="idx"
                class="flex items-center gap-2"
              >
                <input
                  v-model="pos.positionName"
                  placeholder="岗位名称"
                  class="h-8 flex-1 rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700 placeholder-gray-300 focus:border-blue-500 focus:outline-none"
                />
                <button
                  type="button"
                  @click="removePositionTemplate(idx)"
...
import {
  Plus, RefreshCw, Pencil, Trash2, Layers,
  Building, Lock, ChevronRight, Search, Briefcase, X
...
import type { OrgType, OrgTypeTreeNode, OrgCategoryInfo, CreateOrgTypeRequest, UpdateOrgTypeRequest, PositionTemplate } from '@/types/orgType'
import { ORG_CATEGORY_LABELS, ORG_FEATURE_LABELS } from '@/types/orgType'

// --- Feature descriptions ---
const featureDescriptions: Record<string, string> = {
  dataPermissionBoundary: '作为数据权限隔离的边界节点',
  inspectionTarget: '可作为量化检查的目标范围',
  memberManagement: '支持成员（用户）的归属管理',
  attendance: '支持考勤功能',
  scheduling: '支持排课或排班功能'
}

// --- Category colors ---
const CATEGORY_COLORS: Record<string, string> = {
  ROOT: 'bg-blue-50 text-blue-600',
  BRANCH: 'bg-purple-50 text-purple-600',
  FUNCTIONAL: 'bg-green-50 text-green-600',
...
const TypeSidebarNode = defineComponent({
  ...
  props: {
    node: { type: Object as () => OrgTypeTreeNode, required: true },
    ...
    const renderNode = () => {
      const row = h('div', {
        ...
        h('span', { class: 'flex h-6 w-6 flex-shrink-0 items-center justify-center rounded bg-gray-100' }, [
          h(Building, { class: 'h-3.5 w-3.5 text-gray-500' })
        ]),
      ...
      const children = hasChildren.value && isExpanded.value
        ? props.node.children.map((child: OrgTypeTreeNode) =>
            h(TypeSidebarNode, {
              node: child,
              level: props.level + 1,
              selectedId: props.selectedId,
              searchKeyword: props.searchKeyword,
              onSelect: (n: OrgTypeTreeNode) => emit('select', n)
            })
          )
        : []
...
const treeData = ref<OrgTypeTreeNode[]>([])
const flatTypes = ref<OrgType[]>([])
const categories = ref<OrgCategoryInfo[]>([])
...
const addingUnderParentCode = ref<string>('')  // Track parent when adding sub-type
...
const formData = reactive({
  ...
  defaultPositions: [] as PositionTemplate[]
})
...
const activeFeatures = computed(() => {
  ...
  return Object.entries(selectedType.value.features)
    .filter(([_, v]) => v)
    .map(([k]) => ({ key: k, label: ORG_FEATURE_LABELS[k] || k }))
})

// parentTypeOptions removed — parentTypeCode is now auto-derived from allowedChildTypeCodes

// Only show types that: (1) aren't self, (2) aren't already a child of another type
const allTypeCodesForChildSelect = computed(() => {
  const editingCode = isEdit.value ? formData.typeCode : ''
  // Build set of type codes already claimed as children by OTHER types
  const claimedByOthers = new Set<string>()
  for (const t of flatTypes.value) {
    if (t.typeCode === editingCode) continue  // skip self
    if (t.allowedChildTypeCodes) {
      for (const code of t.allowedChildTypeCodes) {
        claimedByOthers.add(code)
      }
    }
  }
  return flatTypes.value.filter(t => {
    if (t.typeCode === editingCode) return false  // can't be own child
    // Allow if: not claimed by others, OR already in our own allowedChildTypeCodes
    return !claimedByOthers.has(t.typeCode) || formData.allowedChildTypeCodes.includes(t.typeCode)
  })
})

const dialogTitle = computed(() => {
  if (isEdit.value) return '编辑组织类型'
  if (addingUnderParentCode.value) {
    const parent = flatTypes.value.find(t => t.typeCode === addingUnderParentCode.value)
    return parent ? `新增子类型 — ${parent.typeName}` : '新增组织类型'
  }
  return '新增组织类型'
})

// --- Tree filter ---
const filterTree = (nodes: OrgTypeTreeNode[], keyword: string): OrgTypeTreeNode[] => {
  if (!keyword) return nodes
  const lower = keyword.toLowerCase()
  return nodes.reduce<OrgTypeTreeNode[]>((acc, node) => {
...
// --- Helpers ---
const flattenTree = (nodes: OrgTypeTreeNode[]): OrgType[] => {
  const result: OrgType[] = []
  const traverse = (list: OrgTypeTreeNode[]) => {
...
// --- Position template helpers ---
const addPositionTemplate = () => {
  formData.defaultPositions.push({
    positionName: '',
    sortOrder: formData.defaultPositions.length
  })
}

const removePositionTemplate = (index: number) => {
  formData.defaultPositions.splice(index, 1)
}
...
const handleSelectNode = (node: OrgTypeTreeNode) => {
...
const handleAdd = (parentTypeCode?: string) => {
  ...
  addingUnderParentCode.value = parentTypeCode || ''
...
const handleEdit = (item: OrgType) => {
  ...
  Object.assign(formData, {
    ...
    defaultPositions: item.defaultPositions ? item.defaultPositions.map(p => ({ ...p })) : []
  })
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    // Clean features: only include keys that are true
    const cleanFeatures = Object.fromEntries(
      Object.entries(formData.features).filter(([_, v]) => v !== undefined)
    )

    if (isEdit.value && currentId.value) {
      const validPositions = formData.defaultPositions.filter(p => p.positionName.trim())
      const updateData: UpdateOrgTypeRequest = {
        ...
        defaultPositions: validPositions.length > 0 ? validPositions : undefined
      }
      await orgTypeApi.update(currentId.value, updateData)
      ElMessage.success('更新成功')
    } else {
      const validPositionsCreate = formData.defaultPositions.filter(p => p.positionName.trim())
      const createData: CreateOrgTypeRequest = {
        typeCode: formData.typeCode,
        typeName: formData.typeName,
        parentTypeCode: addingUnderParentCode.value || undefined,
        category: formData.category || undefined,
        description: formData.description || undefined,
        sortOrder: formData.sortOrder,
        features: Object.keys(cleanFeatures).length > 0 ? cleanFeatures : undefined,
        ...
        defaultUserTypeCodes: formData.defaultUserTypeCodes.length > 0 ? formData.defaultUserTypeCodes : undefined,
        defaultPlaceTypeCodes: formData.defaultPlaceTypeCodes.length > 0 ? formData.defaultPlaceTypeCodes : undefined,
        defaultPositions: validPositionsCreate.length > 0 ? validPositionsCreate : undefined
      }
      await orgTypeApi.create(createData)

      // Auto-add to parent's allowedChildTypeCodes
      if (addingUnderParentCode.value) {
        const parent = flatTypes.value.find(t => t.typeCode === addingUnderParentCode.value)
        if (parent) {
          const existingChildren = parent.allowedChildTypeCodes || []
          if (!existingChildren.includes(formData.typeCode)) {
            await orgTypeApi.update(parent.id, {
              allowedChildTypeCodes: [...existingChildren, formData.typeCode]
            })
          }
        }
      }
