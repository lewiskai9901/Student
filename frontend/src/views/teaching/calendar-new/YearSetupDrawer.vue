<template>
  <div>
    ...
    <div>
      ...
      <table class="tm-table">
        <colgroup><col /><col style="width: 100px" /><col style="width: 100px" /><col style="width: 80px" /><col style="width: 140px" /></colgroup>
        <thead><tr><th class="text-left">学期名称</th><th>开始</th><th>结束</th><th>状态</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-if="semesters.length === 0"><td colspan="5" class="tm-empty">暂无学期</td></tr>
    ...
    <Transition name="tm-drawer">
      <div v-if="semFormVisible" class="tm-drawer-overlay" @click.self="semFormVisible = false">
        <div class="tm-drawer" style="width: 400px;">
          ...
          <div class="tm-drawer-body">
            <div class="tm-section">
              <div class="tm-field"><label class="tm-label">学期名称</label><input v-model="semForm.semesterName" class="tm-input" placeholder="如: 2025-2026第一学期" /></div>
              <div class="tm-field"><label class="tm-label">学期编码</label><input v-model="semForm.semesterCode" class="tm-input" placeholder="如: 2025-2026-1" /></div>
              <div class="tm-field">
                <label class="tm-label">学期类型</label>
                <div class="tm-radios" style="width: 200px;">
                  <label :class="['tm-radio', { active: semForm.semesterType === 1 }]" @click="semForm.semesterType = 1"><input type="radio" />第一学期</label>
                  <label :class="['tm-radio', { active: semForm.semesterType === 2 }]" @click="semForm.semesterType = 2"><input type="radio" />第二学期</label>
                </div>
              </div>
              <div class="tm-fields tm-cols-2">
                <div class="tm-field"><label class="tm-label">开始日期</label><input v-model="semForm.startDate" type="date" class="tm-input" /></div>
                <div class="tm-field"><label class="tm-label">结束日期</label><input v-model="semForm.endDate" type="date" class="tm-input" /></div>
              </div>
            </div>
          </div>
          <div class="tm-drawer-footer">
...
const semForm = ref<any>({ semesterType: 1 })
...
function showSemForm() { semForm.value = { semesterType: 1 }; semFormVisible.value = true }
...
async function saveSem() {
  try { await semesterApi.create(semForm.value); ElMessage.success('创建成功'); semFormVisible.value = false; loadSemesters(); emit('semesterChanged') } catch { ElMessage.error('创建失败') }
}
