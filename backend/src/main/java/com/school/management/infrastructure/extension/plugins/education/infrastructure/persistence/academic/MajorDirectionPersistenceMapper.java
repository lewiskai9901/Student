...
public interface MajorDirectionPersistenceMapper extends BaseMapper<MajorDirectionPO> {
    ...
    @Select("SELECT * FROM major_directions WHERE major_id = #{majorId} AND deleted = 0 ORDER BY sort_order, id")
    ...
    @Select("SELECT * FROM major_directions WHERE enabled = 1 AND deleted = 0 ORDER BY sort_order, id")
