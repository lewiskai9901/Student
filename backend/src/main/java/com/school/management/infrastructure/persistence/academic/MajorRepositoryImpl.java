...
public class MajorRepositoryImpl implements MajorRepository {
    ...
    private MajorPO toPO(Major domain) {
        ...
        po.setMajorCategoryCode(domain.getMajorCategoryCode());
        po.setEnrollmentTarget(domain.getEnrollmentTarget());
        po.setEducationForm(domain.getEducationForm());
        po.setLeadTeacherId(domain.getLeadTeacherId());
        po.setLeadTeacherName(domain.getLeadTeacherName());
        po.setApprovalYear(domain.getApprovalYear());
        po.setMajorStatus(domain.getMajorStatus() != null ? domain.getMajorStatus().name() : null);
    ...
    private Major toDomain(MajorPO po) {
        ...
        if (po.getMajorStatus() != null) {
            try {
                majorStatus = MajorStatus.valueOf(po.getMajorStatus());
        ...
        return Major.builder()
            ...
            .majorCategoryCode(po.getMajorCategoryCode())
            .enrollmentTarget(po.getEnrollmentTarget())
            .educationForm(po.getEducationForm())
            .leadTeacherId(po.getLeadTeacherId())
            .leadTeacherName(po.getLeadTeacherName())
            .approvalYear(po.getApprovalYear())
    ...
    private MajorDirectionPO toDirectionPO(MajorDirection domain, Long majorId) {
        ...
        po.setEnrollmentTarget(domain.getEnrollmentTarget());
        po.setEducationForm(domain.getEducationForm());
        po.setCertificateNames(domain.getCertificateNames());
        po.setTrainingStandard(domain.getTrainingStandard());
        po.setCooperationEnterprise(domain.getCooperationEnterprise());
        po.setMaxEnrollment(domain.getMaxEnrollment());
        po.setSortOrder(domain.getSortOrder());
        po.setEnabled(domain.isEnabled() != null && domain.isEnabled() ? 1 : 0);
        po.setCreatedAt(domain.getCreatedAt());
        po.setUpdatedAt(domain.getUpdatedAt());
        return po;
    }
    ...
    private MajorDirection toDirectionDomain(MajorDirectionPO po) {
        return MajorDirection.reconstitute(
            ...
            po.getCreatedAt(),
            po.getUpdatedAt(),
            po.getEnrollmentTarget(),
            po.getEducationForm(),
            po.getCertificateNames(),
            po.getTrainingStandard(),
            po.getCooperationEnterprise(),
            po.getMaxEnrollment(),
            po.getSortOrder()
        );
    }
