...
public class ResourceRelationUpserter {
    ...
    public void upsert(ResourceRelationDef def, String industry) {
        jdbc.update(
            ...
            " column_name, type_column, ar_relation, resolver_bean, enforce_insert_scope, auto_fill, grants_by_default, subject_column, industry, enabled, tenant_id) " +
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,1) " +
            ...
            " grants_by_default=VALUES(grants_by_default), subject_column=VALUES(subject_column), industry=VALUES(industry), enabled=1",
            ...
            def.autoFill() ? 1 : 0, def.grantsByDefault() ? 1 : 0, def.subjectColumn(), industry);
    }
}
