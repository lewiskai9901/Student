...
import com.school.management.domain.organization.event.GradeCreatedEvent;
import com.school.management.domain.organization.event.GradeStatusChangedEvent;
 ...
 * - 一个部门可以有多个年级的班级
 */
public class Grade extends AggregateRoot<Long> {
    ...
    private Grade(Builder builder) {
        ...
        this.status = builder.status != null ? builder.status : GradeStatus.ENROLLING;
     ...
     * 工厂方法：创建新年级
     */
    public static Grade create(String gradeCode, String gradeName, Integer enrollmentYear,
        ...
        Grade grade = builder()
            ...
            .status(GradeStatus.ENROLLING)
            .createdBy(createdBy)
            .build();

        grade.registerEvent(new GradeCreatedEvent(grade));
        return grade;
    }
    ...
    public void activate(Long updatedBy) {
        if (this.status != GradeStatus.ENROLLING) {
            throw new IllegalStateException("只有招生中的年级可以激活");
        }
        GradeStatus oldStatus = this.status;
        this.status = GradeStatus.ACTIVE;
        this.updatedBy = updatedBy;
        this.updatedAt = LocalDateTime.now();

        registerEvent(new GradeStatusChangedEvent(this, oldStatus, GradeStatus.ACTIVE));
    }
    ...
    public void graduate(Long updatedBy) {
        if (this.status != GradeStatus.ACTIVE) {
            throw new IllegalStateException("只有在读年级可以毕业");
        }
        GradeStatus oldStatus = this.status;
        this.status = GradeStatus.GRADUATED;
        this.updatedBy = updatedBy;
        this.updatedAt = LocalDateTime.now();

        registerEvent(new GradeStatusChangedEvent(this, oldStatus, GradeStatus.GRADUATED));
    }
    ...
    public void stopEnrollment(Long updatedBy) {
        if (this.status == GradeStatus.GRADUATED) {
            throw new IllegalStateException("已毕业年级不能停止招生");
        }
        GradeStatus oldStatus = this.status;
        this.status = GradeStatus.SUSPENDED;
        this.updatedBy = updatedBy;
        this.updatedAt = LocalDateTime.now();

        registerEvent(new GradeStatusChangedEvent(this, oldStatus, GradeStatus.SUSPENDED));
    }
    ...
    public boolean canAddClass() {
        return this.status == GradeStatus.ENROLLING || this.status == GradeStatus.ACTIVE;
    }
    ...
    public boolean isActive() {
        return this.status == GradeStatus.ACTIVE;
    }
    ...
    public boolean isGraduated() {
        return this.status == GradeStatus.GRADUATED;
    }
    ...
    public static class Builder {
        ...
        public Builder status(GradeStatus status) {
            this.status = status;
            return this;
        }
