...
@DisplayName("CorePermissionCatalog 单元测试 (双轨收敛: 纯 holder static API)")
class CorePermissionCatalogTest {

    @Test
    @DisplayName("模块码与模块名固定为 core / 通用核心")
    void moduleMetadata() {
        assertThat(CorePermissionCatalog.MODULE_CODE).isEqualTo("core");
        assertThat(CorePermissionCatalog.MODULE_NAME).isEqualTo("通用核心");
    }

    @Test
    @DisplayName("权限列表非空且每条 code/name 均非空白")
    void permissionsNonBlank() {
        List<PermissionDef> perms = CorePermissionCatalog.permissions();
    ...
    void containsKnownCodes() {
        Set<String> codes = CorePermissionCatalog.permissions().stream()
                .map(PermissionDef::code)
