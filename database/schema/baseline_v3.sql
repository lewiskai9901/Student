...
-- 缺口修复 (2026-06-27): 7 处裸奔读路径补数据权限资源码 (无 created_by 表不含 SELF)。
INSERT INTO `data_resources` (`resource_code`,`resource_name`,`domain_code`,`domain_name`,`enabled`,`sort_order`,`registered_by`,`allowed_scopes`,`subject_relation_filterable`,`resource_kind`,`tenant_id`,`plugin_enabled`) VALUES
...
CREATE TABLE `resource_relations` (
  ...
  `subject_column` varchar(64) DEFAULT NULL COMMENT 'Tier2: owner_org SUBJECT_GRAPH 主表充当 ar.subject_id 的列 (空=默认 id; user_student=user_id)',
...
-- P0-H1: 修补 vestigial 关系种子行的 industry。这些行 registered_by 标了插件但未被任何 PluginPackage.contribute()
-- 贡献 (guardian_of/supervisor_of 概念已合并入 family_of; advisor_of 已 DEPRECATED+禁用), 种子里 industry=NULL
-- → 禁用插件 WHERE industry='COMMON_EXT' 命中不到 = 治理盲区。设为其 tier 对应行业以统一 disable 归属。
-- (它们是历史遗留行, 后续宜评估是否整行删除 —— 单独清理项, 非本次范围。)
UPDATE `relation_types` SET `industry`='COMMON_EXT' WHERE `relation_code` IN ('guardian_of','supervisor_of') AND `industry` IS NULL;
UPDATE `relation_types` SET `industry`='EDU' WHERE `relation_code`='advisor_of' AND `industry` IS NULL;
