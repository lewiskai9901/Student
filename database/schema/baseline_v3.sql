...
-- P0-H1: 关系种子行 industry 兜底。按 tier 给 industry IS NULL 的行补归属, 使禁用插件
-- (WHERE industry='COMMON_EXT'/'EDU') 能统一治理, 且 fresh baseline 即 0 NULL-industry (不依赖启动期自愈)。
-- 覆盖: ① 贡献型 family_of/emergency_contact (启动期 dispatcher 本也会写 COMMON_EXT, 此处先正确)
--       ② vestigial guardian_of/supervisor_of (概念已并入 family_of) ③ advisor_of (DEPRECATED+禁用, tier=DOMAIN→EDU)。
UPDATE `relation_types` SET `industry`='COMMON_EXT' WHERE `tier`='COMMON_EXT' AND `industry` IS NULL;
UPDATE `relation_types` SET `industry`='EDU' WHERE `tier`='DOMAIN' AND `industry` IS NULL;
